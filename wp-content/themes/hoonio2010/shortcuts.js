...
}}var c={global:d+"/search/",downloads:d+"/search/",iphone:d+"/search/",ipoditunes:d+"/search/",mac:d+"/search/""};
var b=c[searchSection]||"/search/";b=b+="?sec="+window.searchSection;enhanceSearch(b);
...
}else{window.onload=function(){a();b()}}}shortcutsPageLoader(loadShortcuts);var SearchShortcut={baseUrl:"http://www.apple.com/global/nav/scripts/shortcuts.php",minimumCharactersForSearch:0,entryDelay:150,currentRequest:false,descriptionCharacters:90,titleCharacters:39,isIe:false,init:function(){this.fullSearchUrl=document.getElementById("globalsearch").getElementsByTagName("form")[0].getAttribute("action");
this.noResults=geoMap.US.noResults;this.viewAll=geoMap.US.viewAll;if(typeof(searchCountry)!="undefined"&&searchCountry){this.noResults=geoMap[searchCountry.toUpperCase()].noResults||this.noResults;
...
document.location=a},search:function(c){var a=this.baseUrl+"?q="+encodeURIComponent(c);
if(typeof(searchSection)!="undefined"&&searchSection){a+="&section="+searchSection
...
c.className="hoverbox";var o=document.createElement("a");o.href=this.fullSearchUrl+"?q="+encodeURIComponent(this.term);
...
if(location.protocol.match(/https/)){d.src="spinner.gif"
}else{d.src="spinner.gif"}d.width="11";
...
};if(navigator.userAgent.match(/AppleWebKit/i)){document.getElementById("sp-searchtext").onkeydown=function(e){var f=typeof(event)!="undefined"?event.keyCode:e.keyCode;
...
SearchShortcut.startKeystrokeTimer()}}}}}}}};function decorateGlobalSearchInput(){var l=document.getElementById("sp-searchtext");
var e=null;var d=0;var i="Search";if(typeof(searchCountry)=="undefined"){searchCountry="us"
}if(geoMap[searchCountry.toUpperCase()].searchText){i=geoMap[searchCountry.toUpperCase()].searchText
}var h="";if(navigator.userAgent.match(/AppleWebKit/i)){if(navigator.userAgent.match(/win/i)){l.className+=" not-round"
}l.setAttribute("type","search");if(!l.getAttribute("results")){l.setAttribute("results",d)
}if(null!=i){l.setAttribute("placeholder",i);l.setAttribute("autosave",h)}l.onblur=function(){if(!SearchShortcut.itemSelected){SearchShortcut.hideResults()
}}}else{l.setAttribute("autocomplete","off");e=document.createElement("input");
l.parentNode.replaceChild(e,l);var b=document.createElement("span");b.className="left";
var j=document.createElement("span");j.className="right";var g=document.createElement("div");
g.className="reset";var a=document.createElement("div");a.className="search-wrapper";
var f=l.value==i;var c=l.value.length==0;if(f||c){l.value=i;a.className+=" blurred empty"
}a.appendChild(b);a.appendChild(l);a.appendChild(j);a.appendChild(g);l.onfocus=function(){var m=a.className.indexOf("blurred")>-1;
if(l.value==i&&m){l.value=""}a.className=a.className.replace("blurred","")};l.onblur=function(){if(!SearchShortcut.itemSelected){SearchShortcut.hideResults()
}if(l.value==""){a.className+=" empty";l.value=i}a.className+=" blurred"};l.onkeydown=function(m){var o=typeof(event)!="undefined"?event.keyCode:m.keyCode;
if(!m){m=event}if(o==13&&!m.altKey){var n=null;if(m.target){n=m.target}else{if(m.srcElement){n=m.srcElement
}}if(n.value.length===0){return false}if(SearchShortcut.selected){SearchShortcut.go(SearchShortcut.selected.url)
}else{SearchShortcut.hideResults();document.getElementById("g-search").submit()
}return}if(l.value.length>=0){a.className=a.className.replace("empty","")}k()};
var k=function(){return(function(m){var n=false;if(!m){m=window.event}if(m.type=="keydown"){if(m.keyCode!=27){return
}else{n=true}}l.blur();l.value="";a.className+=" empty";l.focus()})};g.onmousedown=k();
if(e){e.parentNode.replaceChild(a,e)}}};