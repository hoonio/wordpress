...
if (!class_exists("ezAdSense")) {
  class ezAdSense {
    ...
    function validSize($size) {
      $sizes = $this->defaults['ads']['sizes']['chitika'] ;
    ...
    function mkCode($size, $pop=false) {
      ...
      $rep = $this->splitSize($size) ;
      $needle = array('$x', '$y', '$size', '$kwds') ;
      $rep[] = $size ;
      $kwds = '' ; // $this->mkKwd() ; // For future use
      $rep[] = $kwds ;
      $haystack = $this->defaults['ads']['hay']['chitika'] ;
      if ($pop && !is_user_logged_in()) $haystack = $this->defaults['ads']['hayPop']['chitika'] ;
      $ad = str_replace($needle, $rep, $haystack) ;
      return $ad ;
    }
    ...
    function printAdminPage() {
      ...
      if (isset($_POST['update_ezAdSenseSettings'])) {
        ...
        if (isset($_POST['ezAllowPopunder']))
          $ezAdOptions['allow_popunder'] = 'Yes' ;
        else
          $ezAdOptions['allow_popunder'] = 'No' ;
    ...
    function header_leadin(){
      ...
      global $urCount ;
      $unreal = '' ;
      /* The global count $urCount is not handled properly because
         we are outside the loop, perhaps.
        if ((is_single() || is_page()) && $urCount < $this->urMax)
        $unreal = '<div align="center"><font size="-3">' .
          '<a href="http://wordpress.org/extend/plugins/easy-adsenser/" ' .
          'target="_blank" title="The simplest way to put AdSense to work for you!"> ' .
    ...
    function widget_ezAd_lu($args) {
      ...
      $ezAdOptions = $this->getAdminOptions();
      $ezAdOptions['text_lu'] =
        $this->handleDefaultText($ezAdOptions['text_lu'], '120x90') ;
    ...
    function widget_ezAd_search($args) {
      ...
      $ezAdOptions['text_gsearch'] =
        $this->handleDefaultText($ezAdOptions['text_gsearch'], '120x90') ;
      $metaOptions = $this->widgetMeta() ;
      if (isset($metaOptions['adsense']) && $metaOptions['adsense'] == 'no') return ;
